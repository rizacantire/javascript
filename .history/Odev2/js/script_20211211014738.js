...
function changeCss(e){
    ...
    isActive == (-1) ? (e.target.classList.add("active"),line.textDecoration = "line-through") : (e.target.classList.remove("active"),line.textDecoration = "")
}
...
function newElement() {
  inputText.value = "";
  text.length > 0 ? (success()) : showTostr(".error");
}
function success(){
    let text = inputText.value.trim();
    newItem(text),showTostr(".success")
}
