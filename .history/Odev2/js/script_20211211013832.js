...
function changeCss(e){
    let line = e.target.style.textDecoration = "line-through"
    let notLine = e.target.style.textDecoration = ""
    let targetClassList = e.target.classList
    let isActive = targetClassList.value.search("active")
    isActive == (-1) ? (e.target.classList.add("active"),line) : (e.target.classList.remove("active"),notLine)
